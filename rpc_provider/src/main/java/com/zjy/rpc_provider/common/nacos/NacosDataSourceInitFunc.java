...
public class NacosDataSourceInitFunc implements InitFunc {
    ...
    private void register(String dataIdKey) {
        try {
            ...
            if (null == properties || StringUtils.isBlank(dataId) || StringUtils.isBlank(groupId)) {
            ...
            switch (dataIdKey) {
                case FLOW:
                    ReadableDataSource<String, List<FlowRule>> flowDataSource = new NacosDataSource<>(properties, groupId, dataId,
                            source -> StringUtils.isBlank(source) ? Collections.EMPTY_LIST : JacksonUtils.toObj(source, new TypeReference<List<FlowRule>>() { }));
                ...
                case DEGRADE:
                    ReadableDataSource<String, List<DegradeRule>> degradeDataSource = new NacosDataSource<>(properties, groupId, dataId,
                            source -> StringUtils.isBlank(source) ? Collections.EMPTY_LIST : JacksonUtils.toObj(source, new TypeReference<List<DegradeRule>>() { }));
                ...
                case PARAM_FLOW:
                    ReadableDataSource<String, List<ParamFlowRule>> paramFlowDataSource = new NacosDataSource<>(properties, groupId, dataId,
                            source -> StringUtils.isBlank(source) ? Collections.EMPTY_LIST : JacksonUtils.toObj(source, new TypeReference<List<ParamFlowRule>>() { }));
                ...
                case SYSTEM:
                    ReadableDataSource<String, List<SystemRule>> systemDataSource = new NacosDataSource<>(properties, groupId, dataId,
                            source -> StringUtils.isBlank(source) ? Collections.EMPTY_LIST : JacksonUtils.toObj(source, new TypeReference<List<SystemRule>>() { }));
    ...
    private Properties buildNacosConfigProperties(String serverAddrKey, String namespaceKey, String usernameKey, String passwordKey) {
        ...
        if (StringUtils.isBlank(serverAddress) || serverAddress.startsWith(PARAMS_PREFIX)) {
            return null;
        }
